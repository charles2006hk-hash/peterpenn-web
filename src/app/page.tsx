...
export default function Home() {
  ...
  const [isMenuOpen, setIsMenuOpen] = useState(false); // 控制手機版選單
  ...
  useEffect(() => {
    ...
    const fetchLatestNews = async () => {
      try {
        // 💡 繞過複合索引限制：先依照日期抓取最新 10 筆，再在前端過濾掉封存的
        const q = query(collection(db, "news"), orderBy("date", "desc"), limit(10));
        const snapshot = await getDocs(q);
        const activeNews = snapshot.docs.map(d => d.data()).find(n => !n.isArchived);
        if (activeNews) setLatestNews(activeNews);
      } catch (error) { console.error("載入最新動態失敗:", error); }
    };
    ...
    <main className="relative min-h-screen bg-black text-white selection:bg-white selection:text-black overflow-hidden">
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            ...
            className="fixed inset-0 z-40 bg-black/95 backdrop-blur-lg flex flex-col items-center justify-center space-y-8 font-serif text-xl tracking-[0.3em] uppercase"
          >
      ...
      <section className="relative z-10 min-h-screen flex flex-col items-center justify-center text-center px-4">
        ...
        {latestNews && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 1.2 }}>
            <Link href="/news" className="group flex flex-col md:flex-row items-center gap-3 md:gap-6 border border-zinc-800/60 bg-zinc-900/20 backdrop-blur-sm px-6 py-3 hover:bg-zinc-800/40 hover:border-zinc-500 transition-all duration-700">
      ...
      <section id="contact" className="relative z-10 py-32 px-6 bg-black flex flex-col items-center border-t border-zinc-900">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-2xl w-full text-center">
          ...
          {/* 💡 Web3Forms 免後端寄信：請將 YOUR_ACCESS_KEY 換成你在 Web3Forms 申請的 Key */}
